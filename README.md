...
> - 官方支持范围仅为 Windows x64 与 Linux x86_64；macOS、ARM 和其他架构不受支持。完整矩阵见 [docs/support-matrix.md](docs/support-matrix.md)
...
- **请求路径压缩（dsh-compact）**：在模型请求前按真实 Token 压力执行压缩，
  上下文溢出时最多恢复并重试原请求一次，不再模拟输入 `/compact`。
