...
//
// 测试迁移 .test.ts 后依赖原生 type-stripping 直跑（Node ≥ 24）。
...
const VENDOR_NODE = path.join(__dirname, '..', 'vendor', 'node', process.platform === 'win32' ? 'node.exe' : 'node');
...
if (rt.major < 24) {
  ...
  console.error('[test-runner] 请先运行 npm run fetch-runtime 获取随包 Node，或升级系统 Node 至 24+');
  process.exit(2);
}
