...
function buildPickerOverlayContent() {
  return [
...
function enablePickerBrowseOverlay({ file, fs: fsys = fs, log = () => {} }) {
  ...
  try {
    ...
    log('写入目录选择器降级 overlay 失败: ' + err.message);
    return null;
  }
}
...
// 返回是否实际删除。
function clearAutoPickerBrowseOverlay({ file, fs: fsys = fs, log = () => {} }) {
  try {
    ...
    log('移除目录选择器降级 overlay 失败: ' + err.message);
    return false;
  }
}
